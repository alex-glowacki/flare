...
uint8_t MAG_Init(I2C_HandleTypeDef *hi2c, uint8_t *chip_id)
{
    ...
    /*
     * Write CTRL register (0x0A):
     *   Normal mode, 100 Hz ODR, ±30G range, OSR1=1
     *
     * Note: QMC5883P has NO FBR register (0x0B). Do not write it.
     * Writing 0x0B on this chip writes to an undefined/reserved register.
     */
    ret = MAG_WriteReg(hi2c, QMC5883P_REG_CTRL, QMC5883P_CTRL_VAL);
    if (ret != MAG_OK)
        return ret;
                    ...
                    int16_t *z)
{
     ...
     * QMC5883P data order: XL, XH, YL, YH, ZL, ZH — LSB first, 2's complement.
     *
     * Note: Data starts at 0x01, NOT 0x00 as on QMC5883L.
     */
    uint8_t buf[6] = {0};
    ret = MAG_ReadRegs(hi2c, QMC5883P_REG_XOUT_L, buf, 6);
    if (ret != MAG_OK)
        return MAG_ERR_I2C;
