...
int main(void) {
   ...
   * MAG_Cal_t is zero-initialised — offsets will be 0 until a calibration
   * sweep is performed. Heading will work but will have hard-iron error
   * until calibrated.
   */
  ...
  while (1) {
    ...
    /*
     * Read magnetometer heading. On I2C error or timeout, retain the
     * last valid heading — better than feeding 0 into the filter.
     */
    float new_heading;
    ...
    IMU_Fusion_Update(&imu_fusion, imu_acc_x, imu_acc_y, imu_acc_z, imu_gyr_x,
                      imu_gyr_y, imu_gyr_z, mag_heading, 0.01f, 0.96f, 0.90f);
